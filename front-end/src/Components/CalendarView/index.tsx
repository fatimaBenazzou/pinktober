...
function CalendarView({
	...
	calendarEvents: CalendarFormat[];
	addNewEvent: (day: Date) => void;
	openDayDetail: (params: { filteredEvents: CalendarFormat[]; title: string }) => void;
}) {
	...
	const [events, setEvents] = useState<CalendarFormat[]>([]);
	...
	const openAllEventsDetail = (date: Date, theme: ThemesFormat) => {
			...
			<div className="w-full  bg-base-100 p-4 rounded-lg">
				<div className="flex items-center justify-between">
					<div className="flex  justify-normal gap-2 sm:gap-4">
						<p className="font-semibold text-xl w-48">{moment(firstDayOfMonth).format("MMMM yyyy").toString()}</p>
					...
					<div>
						<button className="btn  btn-sm btn-ghost btn-outline normal-case" onClick={() => addNewEvent(new Date())}>
							Add New Event
						</button>
					</div>
				</div>
